...
// Module-level async mutex — serializes all git operations to prevent
// concurrent agents from corrupting the shared working tree.
let gitLockChain: Promise<unknown> = Promise.resolve();
function withGitLock<T>(fn: () => Promise<T>): Promise<T> {
  const next = gitLockChain.then(fn);
  gitLockChain = next.catch(() => {}); // prevent chain from breaking on error
  return next;
}
...
/**
 * Write file content to `agents/<agentName>/<filename>` on the agent's branch.
...
export async function commitAndPush(
  ...
  return withGitLock(async () => {
    ...
    try {
      ...
      // Push and set upstream on first push (Fix 4: correct array form for simple-git v3)
...
export async function createAgentPR(opts: {
  ...
  // Fix 3: check for an existing open PR to avoid GitHub 422 on duplicates
  const existingUrl =
    `${apiBase}/pulls` +
